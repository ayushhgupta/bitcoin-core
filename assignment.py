...
class ExampleTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # print(self.tip)
        n = self.nodes[1].getblockcount()
        self.log.info("Current number of blocks: {}".format(n))

        self.log.info("Mining new block from node1")
        block = int(self.generate(self.nodes[1], nblocks=1)[0], 16)
        self.block_time = self.nodes[0].getblock(self.nodes[0].getbestblockhash())['time'] + 1
        self.tip = int(self.nodes[1].getbestblockhash(), 16)

        self.log.info("Sending the block to node2")
        self.sync_all()

        self.log.info("Check if node2 recieved the block")
        assert_equal(self.nodes[1].getbestblockhash(), self.nodes[2].getbestblockhash())
        
        n = self.nodes[1].getblockcount()
        self.log.info("Number of blocks after mining one more block: {}".format(n))
